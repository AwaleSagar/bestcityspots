...
export default function AIBriefingSkeleton() {
  return (
    <section role="status" aria-label="Loading AI city briefing" className="space-y-8">
      {/* Section Heading */}
      <div className="space-y-3">
        <h2 className="labelled-rule">
          ...
          <span className="text-accent/70 animate-pulse text-xs font-black tracking-[0.2em] uppercase">
            Generating...
          </span>
        </h2>
      ...
      <div className="border-line bg-background/55 flex w-fit flex-wrap items-center gap-1.5 rounded-2xl border p-1.5 md:rounded-2xl">
        {[
          ...
          return (
            <div
              key={idx}
              className={`flex items-center gap-2 rounded-xl px-4 py-2.5 text-xs font-black tracking-[0.1em] uppercase md:gap-2.5 md:rounded-2xl md:px-5 md:py-3 ${
                idx === 0
      ...
      <div className="atlas-panel space-y-4 rounded-4xl p-8 shadow-2xl md:rounded-4xl md:p-10">
