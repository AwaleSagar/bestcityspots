...
export default function TestimonialsSection() {
  return (
    <section
      className="relative overflow-hidden rounded-[1.5rem] border border-line bg-surface/65 py-8 shadow-3xl backdrop-blur-xl md:rounded-[2rem] md:py-10"
    ...
    >
      ...
      <div className="mx-auto grid max-w-6xl gap-6 px-5 md:px-8 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] lg:gap-10">
        <ScrollReveal animation="fade-up">
          <div className="rounded-[1.4rem] border border-line bg-background/40 p-5 md:p-7 lg:sticky lg:top-28">
            <span className="section-heading">Use the atlas</span>
            <h2
              id="highlights-heading"
              className="mt-3 text-[clamp(1.6rem,3.5vw,2.6rem)] leading-[1] text-foreground"
            >
            ...
            <div className="mt-6 grid gap-2.5 text-sm text-muted">
              <div className="rounded-xl border border-line bg-surface/65 px-3.5 py-2.5">Read the city summary before you start scanning details.</div>
              <div className="rounded-xl border border-line bg-surface/65 px-3.5 py-2.5">Keep pricing, local tips, and signals visible without overwhelming the first screen.</div>
              <div className="rounded-xl border border-line bg-surface/65 px-3.5 py-2.5">Let the product feel more intentional on mobile, not just compressed from desktop.</div>
            </div>
          </div>
        </ScrollReveal>
            ...
            {highlights.map(({ text, label, icon: Icon }, idx) => (
              <ScrollReveal key={label} animation="fade-up" staggerIndex={idx} staggerDelay={0.15}>
                <motion.li
                  ...
                  className="group relative flex h-full flex-col gap-4 rounded-[1.4rem] border border-line bg-surface/70 p-4 transition-all duration-500 hover:border-accent/20 hover:shadow-lg md:p-5"
                >
                  ...
                  <div>
                    <h3 className="mb-1.5 text-base leading-tight text-foreground">
                      {label}
                    </h3>
            ...
            {uniqueFeatures.map(({ icon: Icon, title, description }, idx) => (
              <ScrollReveal key={title} animation="fade-up" staggerIndex={idx} staggerDelay={0.15}>
                <motion.li
                  whileHover={{ y: -6 }}
                  transition={{ type: "spring", stiffness: 400, damping: 20 }}
                  className="group flex h-full flex-col gap-4 rounded-[1.4rem] border border-line bg-background/35 p-4 transition-all duration-500 hover:border-accent/18 hover:shadow-lg md:p-5"
                >
                  ...
                  <div>
                    <h4 className="mb-1.5 text-base leading-tight text-foreground">
                      {title}
                    </h4>
                    <p className="text-[13px] leading-relaxed text-muted">
                      {description}
