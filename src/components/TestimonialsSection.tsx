...
export default function TestimonialsSection() {
  return (
    <section
      className="border-t border-foreground/5 bg-foreground/[0.02] py-16"
      aria-labelledby="testimonials-heading"
    >
      <div className="mx-auto max-w-4xl px-6">
        <h2
          id="testimonials-heading"
          className="mb-10 text-center text-sm font-black uppercase tracking-[0.2em] text-foreground/50"
        >
          What travelers say
        </h2>
        <ul className="grid gap-8 sm:grid-cols-3" role="list">
          {testimonials.map(({ quote, author, context }) => (
            <li
              key={author}
              className="liquid-glass flex flex-col gap-4 rounded-2xl border border-foreground/5 p-6"
            >
              <Quote
