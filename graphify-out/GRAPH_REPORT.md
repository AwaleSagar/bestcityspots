# Graph Report - bestcityspots  (2026-05-10)

## Corpus Check
- 129 files · ~125,617 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1083 nodes · 1670 edges · 46 communities detected
...
1. `publicEnv()` - 25 edges
2. `serverEnv()` - 21 edges
3. `httpFetch()` - 15 edges
4. `main()` - 12 edges
5. `tryClaimPaidProviderUse()` - 12 edges
6. `fetchFreshPlaces()` - 12 edges
7. `main()` - 12 edges
8. `fetchFreshPlaces()` - 12 edges
9. `processAnalyticsBatch()` - 11 edges
10. `setStorageItem()` - 11 edges

## Surprising Connections (you probably didn't know these)
- `shouldTrack()` --calls--> `publicEnv()`  [INFERRED]
  src/components/analytics/AnalyticsProvider.tsx → src/lib/env.ts
- `sitemap()` --calls--> `listMonthSlugs()`  [INFERRED]
  src/app/sitemap.ts → src/lib/topical-hubs.ts
- `generateStaticParams()` --calls--> `listMonthSlugs()`  [INFERRED]
  src/app/best-cities-to-visit-in/[month]/page.tsx → src/lib/topical-hubs.ts
- `AIBriefingSection()` --calls--> `readCachedCityInsight()`  [INFERRED]
  src/app/cities/[slug]/AIBriefingSection.tsx → src/lib/intelligence.ts
- `CoreMetricsCard()` --calls--> `getCityMetrics()`  [INFERRED]
  src/app/cities/[slug]/page.tsx → src/lib/metrics.ts
...
Cohesion: 0.04
Nodes (34): publicEnv(), requirePublicEnv(), requireServerEnv(), serverEnv(), warnOnce(), GET(), HealthTimeoutError, withTimeout() (+26 more)

### Community 1 - "Community 1"
Cohesion: 0.05
Nodes (38): buildFaqs(), FAQContent(), isCacheFresh(), minutes(), CircuitOpenError, HttpError, httpJson(), aqiLabelFromOwm() (+30 more)

### Community 2 - "Community 2"
Cohesion: 0.05
Nodes (45): getToday(), parseReferrer(), parseUserAgent(), processAnalyticsBatch(), recordCityView(), recordDailyVisitorStats(), recordDeviceStats(), recordGeoStats() (+37 more)

### Community 3 - "Community 3"
Cohesion: 0.04
Nodes (27): CityViewTracker(), PageTracker(), findNearest(), findNearestCity(), normalizeQuery(), queryCitiesInBox(), searchCities(), useDeviceType() (+19 more)

### Community 4 - "Community 4"
Cohesion: 0.06
Nodes (60): haversineKm(), getDistanceKm(), isWithinMaxPriceTier(), mapPriceLevel(), matchesQuery(), sortPlaces(), buildPlaceImagePublicUrl(), buildPlacesRequestKey() (+52 more)

### Community 5 - "Community 5"
Cohesion: 0.08
Nodes (47): extractBalancedJson(), generateText(), generateTextStream(), getClient(), sanitizeJsonArrayResponse(), sanitizeJsonResponse(), capRadius(), fetchPhotoBytes() (+39 more)
...
Cohesion: 0.08
Nodes (34): generateSessionId(), getGeoConsent(), getOrCreateSession(), isNewVisitor(), setGeoConsentStorage(), shouldTrack(), handleAccept(), handleDecline() (+26 more)

### Community 8 - "Community 8"
Cohesion: 0.09
Nodes (40): checkInsightsCacheStatus(), checkMetricsCacheStatus(), checkPlacesCacheStatus(), checkWeatherCacheStatus(), createSupabaseClient(), getAqiLabel(), getTopCitiesByPopulation(), getTopCitiesByTraffic() (+32 more)
...
Cohesion: 0.14
Nodes (26): main(), paint(), probe(), runTest(), shouldRunSuite(), skipTest(), suiteConsistency(), suitePerformance() (+18 more)

### Community 11 - "Community 11"
Cohesion: 0.12
Nodes (28): backoffMs(), breakerKey(), CircuitOpenError, getBreaker(), HttpError, httpFetch(), httpJson(), isAbortError() (+20 more)
...
Cohesion: 0.17
Nodes (24): c(), formatNumber(), getCityViews(), getDailyTrend(), getDeviceStats(), getGeoStats(), getOverview(), getTrafficSources() (+16 more)

### Community 15 - "Community 15"
Cohesion: 0.16
Nodes (20): extractErrorMessage(), extractPlaces(), formatSample(), isPlaceResult(), isValidDisplayName(), jsonType(), mapJsonLayout(), parseArgs() (+12 more)
...
Cohesion: 0.15
Nodes (7): sitemap(), isValidMonthSlug(), listMonthSlugs(), monthLabel(), generateMetadata(), generateStaticParams(), generateMetadata()

### Community 20 - "Community 20"
Cohesion: 0.16
Nodes (12): searchPlaces(), fisherYatesShuffle(), getCitiesFromCategory(), getMixedCitiesFromCategories(), GET(), isAbortError(), withTimeout(), GET() (+4 more)
...
Cohesion: 0.19
Nodes (7): coerceToInsight(), pickArray(), pickString(), parsePartialJson(), coerceToInsight(), pickArray(), pickString()
...
Cohesion: 0.6
Nodes (4): main(), parseCsvLine(), main(), parseCsvLine()
...
## Knowledge Gaps
- **23 isolated node(s):** `HealthTimeoutError`, `HealthTimeoutError`, `Cache First TTL Strategy`, `Google Places Bayesian ranking`, `Weather and AQI fusion` (+18 more)
...
- **Why does `publicEnv()` connect `Community 0` to `Community 3`, `Community 4`, `Community 7`, `Community 19`, `Community 30`?**
  _High betweenness centrality (0.099) - this node is a cross-community bridge._
- **Why does `serverEnv()` connect `Community 0` to `Community 1`, `Community 4`, `Community 5`?**
  _High betweenness centrality (0.056) - this node is a cross-community bridge._
- **Why does `formatPopulation()` connect `Community 3` to `Community 2`?**
...
- **Are the 9 inferred relationships involving `serverEnv()` (e.g. with `currentLevel()` and `isStructured()`) actually correct?**
  _`serverEnv()` has 9 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 5 inferred relationships involving `tryClaimPaidProviderUse()` (e.g. with `serverEnv()` and `generateText()`) actually correct?**
  _`tryClaimPaidProviderUse()` has 5 INFERRED edges - model-reasoned connections that need verification._