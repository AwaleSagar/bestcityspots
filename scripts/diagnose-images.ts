...
(async () => {
  ...
  const r = await s.from("city_places_cache").select("city_name,place_type,places_data").limit(5);
  if (r.error) {
  ...
  for (const c of r.data) {
    ...
    for (const p of displayed) {
      console.log(
        `  ${p.imageUrl ? "✓" : "✗"} "${p.displayName?.text}" (${p.userRatingCount} reviews, ★${p.rating}) price=${p.priceLevel ?? "—"}`
      );
    }
