...
async function runProviderTests() {
  ...
  try {
    ...
    if (currentMeteo) {
      check(
        "temperature is defined or null",
        typeof currentMeteo.tempC === "number" || currentMeteo.tempC === null
      );
      check(
        "humidity is defined or null",
        typeof currentMeteo.humidity === "number" || currentMeteo.humidity === null
      );
      check(
        "observedAt date is present",
        typeof currentMeteo.observedAt === "string" && currentMeteo.observedAt.length > 0
      );
    }
    ...
    check(
      "Open-Meteo exceptions are handled",
      false,
  ...
  console.log("OpenWeatherMap Auth Gating & Fallback behavior");
  {
    ...
    if (!apiKey) {
      check(
        "without API key, OpenWeatherMap gracefully resolves to auth error",
        res.ok === false && res.reason === "auth"
      );
    } else {
      check(
        "with API key, OpenWeatherMap query is successful or outage-rejected",
        res.ok === true || res.reason === "outage"
      );
    }
  }
  ...
  console.log("Gemini Provider Gating & Client limits");
  {
    ...
    check(
      "PROMPT_VERSIONS is structured properly",
      PROMPT_VERSIONS.CITY_INSIGHT === 2 && PROMPT_VERSIONS.TRENDING_CITIES === 3
    );

    if (!geminiKey) {
      const res = await generateText("Hello Gemini from testing");
      check(
        "without API key, generateText returns auth reason",
        res.ok === false && res.reason === "auth"
      );
    } else {
      console.log(
        "  → Gemini API key present, skipping expensive synthesis checks to preserve spend."
      );
    }
  }
